...
An action is an endpoint that handles incoming HTTP requests for a specific [route](guides/1.3/routing/overview).
...
That is the object that carries the payload that comes from incoming HTTP requests from the [router](guides/1.3/routing/basic-usage).
