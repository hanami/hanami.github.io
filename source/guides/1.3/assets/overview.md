...
In order to force browsers to cache the right copy of an asset, during the deploy, Hanami creates a copy of each file by [appending its checksum](guides/1.3/command-line/assets) to the file name.
...
If enabled, [assets helpers](guides/1.3/helpers/assets) will generate checksum relative URLs.
...
We can use the `javascript` [helper](guides/1.3/helpers/assets) to include `ember.js` in our application.
