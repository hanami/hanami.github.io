...
We pointed our application's root URL to the `index` action of the `home` controller (see the [routing guide](/guides/head/routing/overview) for more information).
...
Read more about entities and repositories in the [models guide](/guides/head/models/overview).
...
about how migrations work in the [migrations' guide](/guides/head/migrations/overview).
...
action does not actually [_expose_](/guides/head/actions/exposures) the books to our view. We can write a test for
that change:
...
Let's use [form helpers](/guides/head/helpers/forms) to build this form in `apps/web/templates/books/new.html.erb`:
...
container `<div>` using Hanami's [HTML builder helper](/guides/head/helpers/html5).
...
Explore the [other guides](/guides/head), the [Hanami API documentation](http://www.rubydoc.info/gems/hanami), read the [source code](https://github.com/hanami) and follow the [blog](/blog).
