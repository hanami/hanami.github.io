...
An action is an endpoint that handles incoming HTTP requests for a specific [route](/guides/head/routing/overview).
...
That is the object that carries the payload that comes from incoming HTTP requests from the [router](/guides/head/routing/basic-usage).
