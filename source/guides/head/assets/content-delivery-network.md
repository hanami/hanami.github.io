...
Once _CDN mode_ is on, all the [asset helpers](/guides/head/helpers/assets) will return **absolute URLs**.
...
When we [precompile assets](/guides/head/command-line/assets) at deploy time, Hanami calculates the checksum of all our assets and it adds a special HTML attribute `integrity` to our asset tags like `<script>`.
